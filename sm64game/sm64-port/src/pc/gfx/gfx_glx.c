...
static void gfx_glx_init(const char *game_name, bool start_in_fullscreen, bool start_in_foreground) {
    ...
    if (!start_in_foreground) {
        Atom below = XInternAtom(glx.dpy, "_NET_WM_STATE_BELOW", False);

        XChangeProperty(glx.dpy, glx.win, glx.atom_wm_state, XA_ATOM, 32,
